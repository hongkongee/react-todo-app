...
function Chat() {
  ...
  useEffect(() => {
    ...
    console.log('첫번째 useEffect 실행!', userName);
  ...
  // 로그인을 할 때(submit) 아이디 + 선택한 방번호를 서버에 전송
  const onSubmitHandler = (e) => {
    e.preventDefault();
    roomSocket.emit('login', { userId: userName, roomNumber }); // 서버로 아이디, 방번호 전송 (처음에는 이름으로 전달)
    setIsLogin(true);
  };
  ...
  // 아이디 클릭 시 privateTarget 상태변수를 바꾸는 핸들러 (귓속말 모드)
      ...
      <div className="wrap">
        {isLogin ? (
          <div className="chat-box">
            <h3>
              Login as a {userName} in Room {roomNumber}
            </h3>
